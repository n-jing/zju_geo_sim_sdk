...
int build_mesh_with_boundary(void)
{
  ...
  auto new_vi = m.add(typename mesh_t::vert_t(99));
  split_face(m, m.faces().begin(), new_vi);
  ...
  while (e != m.edges().end())
  {
    if (try_edge_flip(m, e))
    {
      ++e;
      continue;
    }
    std::cout << "flip edge (" << e->oppo()->vert()->id_
              << ", " << e->vert()->id_ << ")" << std::endl;
