...
public:
  ...
  T operator()(size_t i, size_t j) const {
    const INT_TYPE *pos = std::find(idx_.begin()+ptr_[j], idx_.begin()+ptr_[j+1], i);
    if(pos == idx_.begin()+ptr_[j+1]) return 0;
    return val_[pos-idx_.begin()];
  }
  T &set(size_t i, size_t j) {
    INT_TYPE *pos = std::find(idx_.begin()+ptr_[j], idx_.begin()+ptr_[j+1], i);
    if(pos == idx_.begin()+ptr_[j+1]) {
      pos = std::find(idx_.begin()+ptr_[j], idx_.begin()+ptr_[j+1], -1);
      if(pos == idx_.begin()+ptr_[j+1])
        throw "no empty slot in csc";
      idx_[pos-idx_.begin()] = i;
    }
    return val_[pos-idx_.begin()];
  }
  const zjucad::matrix::matrix<INT_TYPE> &ptr() const { return ptr_; }
  zjucad::matrix::matrix<INT_TYPE> &ptr() { return ptr_; }

  const zjucad::matrix::matrix<INT_TYPE> &idx() const { return idx_; }
  zjucad::matrix::matrix<INT_TYPE> &idx() { return idx_; }

  const zjucad::matrix::matrix<T> &val() const { return val_; }
  zjucad::matrix::matrix<T> &val() { return val_; }
