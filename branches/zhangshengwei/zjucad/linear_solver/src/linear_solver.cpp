...
{
	auto_ptr<hj::sparse::solver_base> slv;
	...
	opts.put("linear_solver/name.desc","<cholmod, umfpack>");
		...
		slv.reset(direct_solver_A::create(A, opts.get<string>("linear_solver/name.value").c_str()));
	else if(solver_type == "iterative")
		slv.reset(iterative_solver_A::create(A, "laspack"));
	else if(solver_type == "PETsc") {
          static auto_ptr<PETsc> petsc_init(ucreate_PETsc());
          return new linear_solver_imp(ucreate_PETsc_CG(A, opts.get<string>("PETsc/pc.value", "sor").c_str()));
	}
	else
