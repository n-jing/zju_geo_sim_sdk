...
int Gauss_Newton::solve_step(const double *x, double *s, boost::property_tree::ptree &opts)
{
	...
	slv_.reset(linear_solver::create(&JTJ_.val()[0], &JTJ_.idx()[0], &JTJ_.ptr()[0], JTJ_.nnz(), JTJ_.size(1), JTJ_.size(2), opts));
...
//	calc_AAT_(JT_, JTJ_);
	...
	start = clock();
	slv_.reset(linear_solver::create(&JTJ_.val()[0], &JTJ_.idx()[0], &JTJ_.ptr()[0],
																	 JTJ_.nnz(), JTJ_.size(1), JTJ_.size(2), opts));
