...
struct kernel_traits<hj::sparse::csc<SCALAR_TYPE,INT_TYPE> > {
    ...
    static void set(size_t r,size_t c,value_type a,sparse_matrix_type& x) {
        ...
        INT_TYPE i=x.ptr_[c],j;
        for(; i<x.ptr_[c+1]; i++) {
            if(x.idx_[i] == r) {
                x.val_[i]=a;
                return;
            } else if(x.idx_[i] > (INT_TYPE)r)
                break;
        }

        //set ptr_
        for(j=c+1; j<x.ptr_.size(1); j++)
            x.ptr_[j]++;

        //set idx_ and val_ after c
        matrix::matrix<INT_TYPE> idx_new;
        idx_new.resize(x.idx_.size(1)+1,1);
        if(i > 0)
            idx_new(matrix::colon<int64_t,int64_t>(0,i-1),matrix::colon_void())=
                x.idx_(matrix::colon<int64_t,int64_t>(0,i-1),matrix::colon_void());
        if(i < x.idx_.size(1))
            idx_new(matrix::colon<int64_t,int64_t>(i+1,idx_new.size(1)-1),matrix::colon_void())=
                x.idx_(matrix::colon<int64_t,int64_t>(i,x.idx_.size(1)-1),matrix::colon_void());
        idx_new[i]=r;

        matrix::matrix<value_type> val_new;
        val_new.resize(x.val_.size(1)+1,1);
        if(i > 0)
            val_new(matrix::colon<int64_t,int64_t>(0,i-1),matrix::colon_void())=
                x.val_(matrix::colon<int64_t,int64_t>(0,i-1),matrix::colon_void());
        if(i < x.val_.size(1))
            val_new(matrix::colon<int64_t,int64_t>(i+1,val_new.size(1)-1),matrix::colon_void())=
                x.val_(matrix::colon<int64_t,int64_t>(i,x.val_.size(1)-1),matrix::colon_void());
        val_new[i]=a;
    ...
    static void row_scale_coeff(const sparse_matrix_type& x,vector_type& s,value_type thres) {
        zero(s,rows(x));
#define SQR(x) (x)*(x)
        for(INT_TYPE c=0; c<(INT_TYPE)x.val_.size(); c++)
            s[x.idx_[c]]+=SQR(x.val_[c]);
...
#undef SQR
    ...
    static void scale_row(const vector_type& s,sparse_matrix_type& x) {
        for(INT_TYPE c=0; c<x.val_.size(); c++)
            x.val_[c]*=s[x.idx_[c]];
    }
    //matrix builder
    ...
    static void set(size_t r,size_t c,matrix_builder& builder,sparse_matrix_type& x) {
        ...
        if(x.idx_.size() > 0) {
            ...
            for(coord c(0,0); c.second<cols; c.second++) {
                for(colB=x.ptr_[c.second],colE=x.ptr_[c.second+1]; colB<colE; colB++) {
                    c.first=x.idx_[colB];
                    value_type& val=x.val_[colB];
                    val=0.0f;
            ...
            x.idx_.resize((INT_TYPE)idx.size());
            x.val_.resize((INT_TYPE)idx.size());
            std::copy(idx.begin(),idx.end(),x.idx_.begin());
            std::copy(vals.begin(),vals.end(),x.val_.begin());
        }
    }
};
