...
public:
    ...
    void build_AAT(const sparse_matrix_type& A,bool AAT,bool non_singular ) {
        ...
        ((INT_TYPE)A.size(1),
         ...
         (INT_TYPE*) &(A.ptr()[0]),
         (INT_TYPE*)&(A.idx()[0]),
         const_cast<SCALAR_TYPE*>(&(A.val()[0])));
...
private:
    ...
    void to_eigen(const sparse_matrix_type &A, INT_TYPE pre_rows, INT_TYPE pre_cols,std::vector<Eigen::Triplet<SCALAR_TYPE,INT_TYPE> > & trips,bool trans = false) {
        if(trans == false) {
            for(INT_TYPE i = 0; i < A.ptr().size()-1; ++i) {
                for(INT_TYPE j = A.ptr()[i]; j < A.ptr()[i+1]; ++j) {
                    trips.push_back(Eigen::Triplet<SCALAR_TYPE,INT_TYPE>(A.idx()[j]+pre_rows,i+pre_cols,A.val()[j]));
                }
            }
        } else {
            for(INT_TYPE i = 0; i < A.ptr().size()-1; ++i) {
                for(INT_TYPE j = A.ptr()[i]; j < A.ptr()[i+1]; ++j) {
                    trips.push_back(Eigen::Triplet<SCALAR_TYPE,INT_TYPE>(i+pre_rows, A.idx()[j]+pre_cols,A.val()[j]));
                }
            }
        }
    }
