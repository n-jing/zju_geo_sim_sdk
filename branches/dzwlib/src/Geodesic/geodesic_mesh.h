...
inline void Mesh::build_adjacencies()
{
	...
	for(unsigned i=0; i<half_edges.size();)
	{
		...
		if(e.length() <= 1e-100)
		  std::cout << e.adjacent_vertices()[0]->id() << " " << e.adjacent_vertices()[1]->id() << std::endl;
	...
	for(unsigned i=0; i<m_faces.size(); ++i)
	{
		...
		for(unsigned j=0; j<3; ++j)		//compute angle adjacent to the vertex j
		{
			...
			if(angle <= 1e-5)
			  std::cout << i << " : " << f.adjacent_vertices()[0]->id() << " " << f.adjacent_vertices()[1]->id() << " " << f.adjacent_vertices()[2]->id() << std::endl;
