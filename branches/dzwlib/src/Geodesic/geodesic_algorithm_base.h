...
public:
  ...
  virtual void trace_back(SurfacePoint& destination,		//trace back piecewise-linear path
      ...
      std::vector<std::pair<unsigned, unsigned> >& edge_pair) = 0;

  void geodesic(SurfacePoint& source,
      SurfacePoint& destination,
      std::vector<SurfacePoint>& path,
      std::vector<std::pair<unsigned, unsigned> >& edge_pair); //lazy people can find geodesic path with one function call
  ...
  void geodesic(std::vector<SurfacePoint>& sources,
      ...
      std::vector<std::pair<unsigned, unsigned> >& edge_pair); //lazy people can find geodesic paths with one function call
    ...
    std::vector<std::pair<unsigned, unsigned> >& edge_pair) //lazy people can find geodesic path with one function call
{
  ...
  trace_back(destination, path, edge_pair);
}
    ...
    std::vector<std::pair<unsigned, unsigned> >& edge_pair) //lazy people can find geodesic paths with one function call
{
	...
	{
		trace_back(destinations[i], paths[i], edge_pair);
	}
}
