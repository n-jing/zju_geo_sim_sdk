...
namespace jtf{
  namespace mesh{
    ...
    int edge2cell_adjacent::init(const matrixst & mesh, const bool & show_debug_info)
    {
      ...
      for(size_t cidx = 0; cidx < mesh.size(2); ++cidx) {
          for(size_t vidx = 0; vidx < mesh.size(1); ++vidx) {
              ...
              if(it != adj_map.end()) {
                  ...
                  cell_idx_[0] = cidx;
                  adj_map.insert(make_pair(make_pair(edge[0],edge[1]),cell_idx_));
                }
      ...
      for(size_t eidx = 0; eidx < edgenum; ++eidx,++mit) {
          ...
          if(mit->second.size() == 2) {
              edge2cell_[eidx].first = mit->second[0];
              edge2cell_[eidx].second = mit->second[1];
            } else if(mit->second.size() == 1) {
              edge2cell_[eidx].first = mit->second[0];
              edge2cell_[eidx].second = -1;
            } else {
              cerr << "# [error] strange edge " << mit->first.first << "," << mit->first.second
                   << endl;
              cerr << "# [error] this edge associates with " << mit->second.size()
                   << " faces: ";
              copy(mit->second.begin(), mit->second.end(), ostream_iterator<size_t>(cerr, ","));
              cerr << endl;
              return __LINE__;
            }
        }
    ...
    int reorder_face(matrixst & face)
    {
      unique_ptr<jtf::mesh::edge2cell_adjacent> ea(
            jtf::mesh::edge2cell_adjacent::create(face));
      if(!ea.get()){
        ...
        const edge2cell_adjacent & ea)
    {
      ...
      for(size_t t = 0 ; t < one_face.size(); ++t){
          ...
          p2f_[one_face[t]].push_back(face_pair.first);
          p2f_[one_face[t]].push_back(face_pair.second);

          p2f_[one_face[(t+1)%one_face.size()]].push_back(face_pair.first);
          p2f_[one_face[(t+1)%one_face.size()]].push_back(face_pair.second);
        }
    }

    void one_ring_face_at_point::sort_int_loop(
        const matrixst &face,
        const matrixd & node)
    {
      ...
      for(size_t fi = 0; fi < faces.size(2); ++fi){
          for(size_t pi = 0; pi < faces.size(1); ++pi){
              for(size_t lp = 0; lp < faces.size(1) -1 ; ++lp)
                p2p_[faces(pi,fi)].push_back(faces((pi+lp+1)%faces.size(1),fi));
            }
        }

      for(p2p_type::iterator it = p2p_.begin(); it != p2p_.end(); ++it){
          vector<size_t> & around_points = it->second;
          set<size_t> temp_set(around_points.begin(), around_points.end());
          around_points.resize(temp_set.size());
          copy(temp_set.begin(), temp_set.end(), around_points.begin());
        }
      return 0;
    }
        ...
        const matrixst & tet)
    {
      ...
      for(one_ring_tet_at_edge::e2tet_type::const_iterator cit = ortae.e2t_.begin();
          ...
          for(set<size_t>::const_iterator scit = one_ring_points.begin();
              ...
              e2f_[cit->first].push_back(face_idx);
            }
        }
    }
