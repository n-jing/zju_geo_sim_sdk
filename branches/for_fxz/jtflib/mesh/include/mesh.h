  ...
  namespace mesh
  {
    ...
    public:
      ...
      static inline bool is_boundary_edge(const std::pair<size_t,size_t> &nb_tri_id) {
        return (nb_tri_id.first == (size_t)-1) ^ (nb_tri_id.second == (size_t)-1);
      }
    ...
    class edge2cell_adjacent_general
    {
    public:
    ...
    //! @breif store N ring face around a point, each ring will be stored
    //!        in a vector, so for each point there will be N vectors in map.
    class one_ring_face_at_point
    {
    public:
      void add_all_faces(const matrixst & faces,
                         const edge2cell_adjacent & ea)
      {
        for(int t = 0; t < faces.size(2); ++t)
      ...
      ///
      void sort_int_loop(const matrixst &face,
                         const matrixd & node);
    ...
    class dyn_one_ring_face_at_point
    {
    public:
    ...
    ///
    int get_boundary_edge(const edge2cell_adjacent& ea,
                          matrixst &edge);
    ...
    public:
      ...
      static inline bool is_outside_face(const std::pair<size_t, size_t> &nb_tet_id) {
        return (nb_tet_id.first  == (size_t)-1) ^ (nb_tet_id.second == (size_t)-1);
      }
    ...
    public:
      ...
      {
        return (loop.front() == (size_t)-1?false:true);
      }
    ...
    public:
      ...
      static inline bool is_outside_face(const std::pair<size_t,size_t> &nb_hex_idx){
        return (nb_hex_idx.first == (size_t)-1) ^ (nb_hex_idx.second == (size_t)-1);
      }
