...
//! @brief subdivide a face in topology from barycenter of its vertices
//! @return iterator of new added vertex.
template <typename TMPL>
const typename TMPL::verts_t::const_iterator
split_face_from_center(mesh_tmpl<TMPL> &m, const typename TMPL::faces_t::const_iterator &f);

//! @brief split a face from a given vertex
template <typename TMPL>
const typename TMPL::verts_t::const_iterator
split_face(mesh_tmpl<TMPL> &m, const typename TMPL::faces_t::const_iterator &f, const typename TMPL::verts_t::const_iterator &v);

//! @brief split edges
//! @NOTE: To save the relation between original edge and its split edges
//! class internal property must has member named split_info,
//! and split_info.root should be initiated as -1.
template <typename TMPL, typename RND_ITR>
bool
split_edges(mesh_tmpl<TMPL> &m, const RND_ITR &vert_loop_beg, std::size_t n);
