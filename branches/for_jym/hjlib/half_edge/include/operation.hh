...
int try_collapse(mesh_tmpl<TMPL> &m, const typename TMPL::edges_t::const_iterator &e)
{
  ...
  do
  {
    do
    {
      //if there are more than two couple of edge share same end vertex
      if (sitr->oppo()->vert() == eitr->oppo()->vert())
      {
        if (sitr != e->prev() && sitr != e->oppo()->next()->oppo()
            && eitr != e->next()->oppo() && eitr != e->oppo()->prev())
          return 1;
      }
      eitr = eitr->next()->oppo();
    }
...
collapse_edge(mesh_tmpl<TMPL> &m, const typename TMPL::edges_t::const_iterator &e)
{
  ...
  //3. a basic thought, do not change topology info unless it's necessary
  ...
  edge_itr_t v_ei = s_vert->edge();
  do
  {
    m()[v_ei].vert() = e_vert;
    v_ei = v_ei->next()->oppo();
  }
  while (v_ei != s_vert->edge());
...
split_face(mesh_tmpl<TMPL> &m, const typename TMPL::faces_t::const_iterator &f, const typename TMPL::verts_t::const_iterator &v)
{
  ...
  {
    CEI new_out_ei = m().add(typename TMPL::edge_t());
    CEI new_in_ei = m().add(typename TMPL::edge_t());
    CFI new_fi = m().add(typename TMPL::face_t());
...
template <typename TMPL>
const typename TMPL::verts_t::const_iterator
split_face_from_center(mesh_tmpl<TMPL> &m, const typename TMPL::faces_t::const_iterator &f)
{
  typedef typename TMPL::verts_t::const_iterator CVI;
  ...
  CVI center_v = m().add(typename TMPL::vert_t());
...
/*/! @brief add a face by existing half edges
template <typename TMPL>
bool
add_face_by_edge(mesh_tmpl<TMPL> &m, const RND_ITR &edge_loop_beg, size_t n)
{
  typedef TMPL::edges_t::const_iterator CEI;
  typedef TMPL::faces_t::const_iterator CFI;

  CEI cur_e = edge_loop_beg[0];
  CFI f = m().add(typename TMPL::face_t());
  m()[f].edge() = cur_e;

  for (size_t i=0; i<n; ++i)
  {
    cur_e = edge_loop_beg[i];
    m()[cur_e].next() = edge_loop_beg[(i+1)%n];
    m()[cur_e].prev() = edge_loop_beg[(i+n-1)%n];
    m()[cur_e].face() = f;
  }
  return true;
}
...
//! @brief split a face by one edge
template <typename TMPL>
bool
split_face_by_edge(mesh_tmpl<TMPL> &m, const typename TMPL::edges_t::const_iterator &e)
{
  ...
  //face not null means this edge has already been coped
  if (e->face())
    return true;
  ...
  //use local variable to improve efficiency
  CVI v1 = e->vert();
  CVI v2 = e->next()->next()->vert();
  CVI v3 = e->prev()->prev()->vert();

  std::vector<CVI> vi_list;
  assert(!v1->edge());

  if (v2->edge() && v3->edge())//split to two
  {
    //NOTE: v.edge should not be null before call add_face2
    m()[v1].edge() = e;
    ...
    //face#1
    vi_list.push_back(v1);
    vi_list.push_back(e->next()->vert());
    vi_list.push_back(v2);
    //face#2
    vi_list.push_back(v1);
    vi_list.push_back(v2);
    vi_list.push_back(e->prev()->vert());

    for (size_t i=0; i<2; ++i)
    {
      add_face2(m, &(vi_list[i*3]), 3);
    }
  }
  else if (!v2->edge() && !v3->edge())//split to four
  {
    m()[v1].edge() = e;
    m()[v2].edge() = e->next()->next();
    m()[v3].edge() = e->prev()->prev();
    ...
    vi_list.push_back(v1);
    vi_list.push_back(v2);
    vi_list.push_back(v3);
    //face#3
    vi_list.push_back(v2);
    vi_list.push_back(e->prev()->prev()->prev()->vert());
    vi_list.push_back(v3);
    //face#4
    vi_list.push_back(v1);
    vi_list.push_back(v3);
  ...
  else if (v2->edge() && !v3->edge())
  {
    ...
    pt_coord norm1 = v1->coord_ - v4->coord_;
    pt_coord norm2 = v3->coord_ - v4->coord_;
    dot_1 = dot(norm1/norm(norm1), norm2/norm(norm2));

    norm1 = v2->coord_ - v3->coord_;
    norm2 = v4->coord_ - v3->coord_;
    dot_2 = dot(norm1/norm(norm1), norm2/norm(norm2));
    ...
    if (dot_1 < dot_2)//an edge links v4 and v2
    {
      //face#2
      vi_list.push_back(v1);
      vi_list.push_back(v2);
      vi_list.push_back(v4);
      //face#3
      vi_list.push_back(v2);
      vi_list.push_back(v3);
      vi_list.push_back(v4);
  ...
  else if (!v2->edge() && v3->edge())
  {
    ...
    pt_coord norm1 = v1->coord_ - v4->coord_;
    pt_coord norm2 = v2->coord_ - v4->coord_;
    dot_1 = dot(norm1/norm(norm1), norm2/norm(norm2));

    norm1 = v4->coord_ - v2->coord_;
    norm2 = v3->coord_ - v2->coord_;
    dot_2 = dot(norm1/norm(norm1), norm2/norm(norm2));
    ...
    if (dot_1 < dot_2)//an edge links v4 and v3
    {
      //face#2
      vi_list.push_back(v1);
      vi_list.push_back(v4);
...
split_edges(mesh_tmpl<TMPL> &m, const RND_ITR &edge_loop_beg, std::size_t n)
{
  ...
  for (size_t i=0; i<n; ++i)
  {
    ...
    CVI vi = m().add(typename TMPL::vert_t());
    
    //create new edges
    CEI new_e_1 = m().add(typename TMPL::edge_t());
    CEI new_e_2 = m().add(typename TMPL::edge_t());
    CEI new_o_1 = m().add(typename TMPL::edge_t());
    CEI new_o_2 = m().add(typename TMPL::edge_t());
...
template <typename TMPL>
bool mesh_simp(mesh_tmpl<TMPL> &m)
{
  //set up a heap, compare edge through function return value
