...
{
	ann_search_ctx ctx;
	ctx.ANNkdDim = dim;						// copy arguments to static equivs
	ctx.ANNkdQ = q;
	ctx.ANNkdPts = pts;
	...
	ctx.ANNkdMaxErr = ANN_POW(1.0 + eps);
	ANN_FLOP(2)							// increment floating op count

	ctx.ANNkdPointMK = new ANNmin_k(k);		// create set for closest k points
										// search starting at the root
	root->ann_search(annBoxDistance(q, bnd_box_lo, bnd_box_hi, dim), ctx);

	for (int i = 0; i < k; i++) {		// extract the k-th closest points
		dd[i] = ctx.ANNkdPointMK->ith_smallest_key(i);
		nn_idx[i] = ctx.ANNkdPointMK->ith_smallest_info(i);
	}
	delete ctx.ANNkdPointMK;				// deallocate closest point set
}
...
void ANNkd_split::ann_search(ANNdist box_dist, ann_search_ctx &ctx)
{
										// check dist calc term condition
	if (ctx.ANNmaxPtsVisited != 0 && ctx.ANNptsVisited > ctx.ANNmaxPtsVisited) return;

										// distance to cutting plane
	ANNcoord cut_diff = ctx.ANNkdQ[cut_dim] - cut_val;

	if (cut_diff < 0) {					// left of cutting plane
		child[ANN_LO]->ann_search(box_dist, ctx);// visit closer child first

		ANNcoord box_diff = cd_bnds[ANN_LO] - ctx.ANNkdQ[cut_dim];
		...
		if (box_dist * ctx.ANNkdMaxErr < ctx.ANNkdPointMK->max_key())
			child[ANN_HI]->ann_search(box_dist, ctx);

	}
	else {								// right of cutting plane
		child[ANN_HI]->ann_search(box_dist, ctx);// visit closer child first

		ANNcoord box_diff = ctx.ANNkdQ[cut_dim] - cd_bnds[ANN_HI];
		if (box_diff < 0)				// within bounds - ignore
			box_diff = 0;
										// distance to further box
		box_dist = (ANNdist) ANN_SUM(box_dist,
				ANN_DIFF(ANN_POW(box_diff), ANN_POW(cut_diff)));
										...
										// visit further child if close enough
		if (box_dist * ctx.ANNkdMaxErr < ctx.ANNkdPointMK->max_key())
			child[ANN_LO]->ann_search(box_dist, ctx);
...
void ANNkd_leaf::ann_search(ANNdist box_dist, ann_search_ctx &ctx)
{
	...
	min_dist = ctx.ANNkdPointMK->max_key(); // k-th smallest distance so far

	for (int i = 0; i < n_pts; i++) {	// check points in bucket

		pp = ctx.ANNkdPts[bkt[i]];			// first coord of next data point
		qq = ctx.ANNkdQ;					// first coord of query point
		dist = 0;

		for(d = 0; d < ctx.ANNkdDim; d++) {
		...
		if (d >= ctx.ANNkdDim &&					// among the k best?
		   (ANN_ALLOW_SELF_MATCH || dist!=0)) { // and no self-match problem
												// add it to the list
			ctx.ANNkdPointMK->insert(dist, bkt[i]);
			min_dist = ctx.ANNkdPointMK->max_key();
		}
	}
