	...
	double				eps)			// error bound (ignored)
{
	...
	while (ctx.ANNPrBoxPQ->non_empty() &&
		(!(ctx.ANNmaxPtsVisited != 0 && ctx.ANNptsVisited > ctx.ANNmaxPtsVisited))) {
...
void ANNkd_split::ann_pri_search(ANNdist box_dist, annkPriSearch_ctx &ctx)
{
	ANNdist new_dist;					// distance to child visited later
										// distance to cutting plane
	ANNcoord cut_diff = ctx.ANNPrQ[cut_dim] - cut_val;

	if (cut_diff < 0) {					// left of cutting plane
		ANNcoord box_diff = cd_bnds[ANN_LO] - ctx.ANNPrQ[cut_dim];
		...
		ANNcoord box_diff = ctx.ANNPrQ[cut_dim] - cd_bnds[ANN_HI];
		if (box_diff < 0)				// within bounds - ignore
			box_diff = 0;
										// distance to further box
		new_dist = (ANNdist) ANN_SUM(box_dist,
...
void ANNkd_leaf::ann_pri_search(ANNdist box_dist, annkPriSearch_ctx &ctx)
{
